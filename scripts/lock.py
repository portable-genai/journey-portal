...
`tag = commit` map that `tests/test_lockfile_pins.py` checks the pins against.
...
#: (extras, output file) for EVERY pyproject-derived lockfile the repo ships, and "every" is
#: load-bearing.
#:
#: `requirements-demo.lock` was not here. `make lock` recompiled it with a raw `uv pip compile`
#: line of its own, so whoever bumped the commons by running THIS script moved two lockfiles and
#: left the third behind: it sat pinning superseded kit releases while pyproject.toml declared
#: newer ones. The CI job that builds the headless browser evidence installs it on top of
#: `requirements-dev.lock`, so it silently DOWNGRADED the eval kit the dev lock had just
#: installed correctly, with no diff anywhere to notice. One regeneration path, and
#: `tests/test_lockfile_pins.py` fails the gate if a lockfile ever escapes it again.
#:
#: `requirements-build.lock` is deliberately absent: it compiles from `requirements-build.in`
#: (hatchling only, no commons, no header to destroy), and `make lock` still recompiles it.
_TARGETS: tuple[tuple[tuple[str, ...], str], ...] = (
...
#
#   uv pip compile pyproject.toml {extras} --universal -o {output}
#
...
#
# tag -> commit, and `tests/test_lockfile_pins.py` proves each line against the tag
# `pyproject.toml` names and against every other lockfile:
{tag_map}
"""


def _declared_tags(extras: tuple[str, ...]) -> dict[str, str]:
    """Each commons package mapped to the tag `pyproject.toml` declares for it.

    Scoped to ONE lockfile: the core dependencies plus exactly the extras that lockfile is
...
def _header_for(
    extras: tuple[str, ...], output: str, tags: dict[str, str], commits: dict[str, str]
) -> str:
...
def main() -> int:
    for extras, output in _TARGETS:
        tags = _declared_tags(extras)
        path = _REPO_ROOT / output
        previous = path.read_text(encoding="utf-8") if path.exists() else None
        command = ["uv", "pip", "compile", "pyproject.toml"]
        for extra in extras:
            command.extend(("--extra", extra))
        command.extend(("--universal", "-o", output))
        ...
        path.write_text(_header_for(extras, output, tags, commits) + body + "\n", encoding="utf-8")

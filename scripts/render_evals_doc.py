...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_journeys.jsonl"
...
def _exercised_block() -> list[str]:
    ...
    counts: dict[str, int] = {}
    for case in cases:
        counts[str(case["kind"])] = counts.get(str(case["kind"]), 0) + 1
    lines = [
        BLOCKS[1],
        "",
        f"{len(cases)} golden cases in `eval/datasets/golden_journeys.jsonl`, each carrying its",
    ...
    for metric, kind in sorted(run_eval.METRIC_KINDS.items()):
        lines.append(f"| `{metric}` | `{kind}` | {counts.get(kind, 0)} |")
    lines.append("")
    return lines

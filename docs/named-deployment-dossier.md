...
title: `journey-portal` named deployment
description: Non-secret decision and evidence record for the journey portal acting as the `cdd-sow-research` Mode 5 BFF.
status: draft
---

# `journey-portal` named deployment dossier

This dossier is the entry gate for one real `journey-portal` installation, and specifically for the half of
`cdd-sow-research`'s cross-origin embedded grant (Mode 5) that this repo owns. `cdd-sow-research`'s own dossier names the
portal's origin as the BFF and pins its `private_key_jwt` client against a JWK set this portal
publishes, so until this file is complete `cdd-sow-research`'s Mode 5 cannot be signed off.
...
| Evidence approver | Ashish Awasthi | Independent reviewer. **NOT independent; see the recorded deviation in the `cdd-sow-research` dossier** |
...
terminate at the same person. The identical decision is recorded in `cdd-sow-research`'s dossier.

## 2. Cloud, residency and edge

| Input | Required value | Review rule |
|---|---|---|
| GCP organization and project | The reference deployment SHARES `cdd-sow-research`'s project, decided by the apply rather than on paper. One consequence is load-bearing: a project belongs to exactly one regular VPC-SC perimeter, and the `cdd-sow-research` stack owns it, so this stack configures none | Dedicated or explicitly approved shared project |
...
| DNS managed zone | `PENDING`, per `cdd-sow-research`'s dossier | Change window recorded |
...
record, taken 2026-08-23 and **REVISED TWICE**: to `us-central1` on 2026-08-24, and back to
`asia-southeast1` on 2026-08-27 once the deferred per-service availability check was finally run.
The launch set CO-LOCATES, and the target region is now `asia-southeast1`. Deploying any service
outside it is an exception needing the service named, the reason it cannot run in-region, a
data-flow record and approval. Read the revisions, not only the original date: the three say
different things.
...
under `us-central1`, on the condition that it returned the moment another target region was
...
The portal authenticates to `cdd-sow-research`'s grant endpoint with `private_key_jwt` (RFC 7523). The
...
| Input | Required value | Review rule |
|---|---|---|
| BFF client id | `PENDING` (registered with `cdd-sow-research` as a distinct service identity; goes in `PORTAL_DOC1_BFF_CLIENT_ID` and must equal what `cdd-sow-research` registers) | Registered service identity |
| BFF authentication method | `private_key_jwt` | mTLS or `private_key_jwt` |
| Signature algorithm | `RS256` (`cdd-sow-research` registers RS256 or ES256; this repo publishes and signs RS256) | Reviewed algorithm |
| Assertion audience | The EXACT `cdd-sow-research` grant endpoint URL, compared as a string by `cdd-sow-research`'s verifier | Exact endpoint, never an origin or prefix |
| Assertion lifetime and skew | 60 seconds, matching `cdd-sow-research`'s registered maximum; the minter refuses a longer one at construction | Reviewed bounded values |
...
| Active `kid` | `PENDING` (goes in `PORTAL_BFF_SIGNING_KID`; it is what the JWK set and every assertion header carry, and what `cdd-sow-research` pins) | Stable across restarts |
...
**Why the rotation dates matter to `cdd-sow-research` and not only here.** `cdd-sow-research`'s dossier section 5 tracks the
...
| Input | Required value | Review rule |
|---|---|---|
| JWKS URL | `PENDING` (the exact path must match what is recorded in `cdd-sow-research`'s dossier; changing it breaks a reviewed registration) | Stable, public, no authentication |
...
| Egress allowance for `cdd-sow-research` | `PENDING` (`cdd-sow-research`'s environment must be allowed to fetch this URL) | Restricted to approved hosts |

## 5. Session binding and user-intent evidence

`cdd-sow-research`'s broker refuses a grant unless the host proves a real user, in a real session, asked for
...
slot or a log line on `cdd-sow-research`. `cdd-sow-research` then re-validates all of it: the duplication is deliberate,
...
| CSRF | A stateless token bound to the session binding AND to the exact method and path, 90-second lifetime, constant-time comparison, no server-side state. `cdd-sow-research`'s `api/csrf.py` is the reference | `tests/test_csrf_and_host_proof.py` |
| Session source subject | The proof carries the PORTAL-VERIFIED principal's subject, never anything the client sent. `cdd-sow-research` requires it to equal the subject it independently derives from the subject token | `tests/test_doc1_grant_route.py` |
| Session binding | A SHA-256 hex digest keyed on the session signing key over the length-prefixed subject and tenant, so `cdd-sow-research` can correlate a session without ever receiving an identifier | Same |
| User-intent id | A fresh opaque value per request, matching `cdd-sow-research`'s `^[A-Za-z0-9._~:-]{16,256}$` | Same |
...
**Open input: the subject must be the same person on both sides.** `cdd-sow-research` compares
...
`cdd-sow-research`'s reviewed installation accepts an OIDC ID token under the Google profile: issuer
...
| Input | Required value | Review rule |
|---|---|---|
| Dedicated Google OAuth client id | `PENDING` (the same client `cdd-sow-research`'s dossier section 5 records as `PENDING`; it is a single input that unblocks both sides) | Distinct broker audience, used for nothing else |
...
send a token from the wrong client, and the exchange would fail at `cdd-sow-research` with a message pointing
at the wrong side of the boundary.

## 7. Promotion and evidence pack

The evidence pack for a Mode 5 sign-off must contain, in addition to the standard `journey-portal` items in
...
- a cross-repo verification record: an assertion minted by the deployed portal accepted by `cdd-sow-research`'s
...
- adversarial grant evidence: cross-site origin, missing CSRF token, a CSRF token from another
  session, and a navigation-destination request, each refused with NO outbound call to `cdd-sow-research`;

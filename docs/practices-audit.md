...
| Check | Verdict | Evidence |
|---|---|---|
| E1 offline eval | N-A | `journey-portal` is non-agentic, so `model-quality-gate` is not a promotion authority for a model. It still runs deterministic journey, routing, identity and tenant-policy isolation metrics in CI. |
| E2 safety false-green | N-A | `journey-portal` has no PII safety metric. Its analogous identity and tenant-policy isolation metrics have strict 0.99 thresholds and tests proving each can fail. |

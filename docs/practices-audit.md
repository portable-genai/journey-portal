# `journey-portal` common-base practices audit
...
| A7 kernel/vertical boundary | PASS | `domain/models.py` is the stable value-object kernel; `catalog.py` and `identity_injection.py` contain `journey-portal` behavior; `ARCHITECTURE.md` and `docs/ADOPTING.md` name the boundary. |
| A8 consume horizontals | PASS | `journey-portal` hosts verticals and contains no guardrail, RAG, registry, evaluation engine or observability implementation to duplicate. The `platform` access-audit adapter is a thin authenticated delegate to `agent-observability` `/v1/audit`; deterministic domain mapping keeps the payload content-free. |

## B. Determinism and LLM boundary

| Check | Verdict | Evidence |
|---|---|---|
| B1 deterministic decisions | N-A | `journey-portal` has no model, score, eligibility or consequential decision engine. Routing and header plans are deterministic domain functions. |
| B2 citations | N-A | `journey-portal` generates no claims or narrative. Embedded applications own citations. |
| B3 maker-checker | N-A | `journey-portal` makes no consequential output; it composes `human-review-console` for review. |
...
| B5 open taxonomy | N-A | `journey-portal` has no regulated category taxonomy or engine keyed by category. |
...
| C2 object/tenant authorization | N-A | `journey-portal` owns no business objects or data store. Each embedded API remains responsible for object and tenant authorization after re-verifying identity. |
| C3 redact first | N-A | `journey-portal` has no agent/model boundary and does not parse or persist proxied bodies. |
| C4 jurisdiction PII packs | N-A | `journey-portal` performs no PII detection or model safety scoring. `agent-guardrail-gateway` and embedded applications own this control. |
...
| C8 owned login | N-A | `journey-portal` owns no browser login or session protocol. The named deployment uses IAP and verifies its assertion audience. |
...
| Check | Verdict | Evidence |
|---|---|---|
| E1 offline eval | PASS | `journey-portal` is non-agentic, so `model-quality-gate` is not a promotion authority for a MODEL, but it is one for this service: `journey-portal` is now a registered bundle and the runner's documented default names it. The gate runs seven deterministic invariants in CI, and their thresholds are no longer unlabelled constants: this repository had no rubric directory at all, and every bar now lives in `eval/rubrics/*.yaml` beside the argument for it, with `assert_covers` failing the build in both directions. |
| E2 safety false-green | PASS | `journey-portal` has no PII safety metric. Its analogous isolation metrics now gate at exactly 1.0, raised from 0.99: over corpora of four and six cases a 0.99 bar was arithmetically identical to 1.0, and it is also what the properties mean, since a trust boundary that holds 99% of the time is a boundary with a hole in it. Two invariants that were covered only by unit tests are now scored kinds: `csrf_token_integrity` and `host_proof_integrity`, the two things standing between a hostile page and a grant made in a signed-in user's name. Both corpora carry refusals as well as acceptances, because a one-directional corpus scores a verifier that accepts everything at a perfect 1.000. |
...
| E4 nothing-evaluated fails closed | PASS | `agent_eval_kit.EvalReport.passed` requires `n_examples > 0` AND a non-empty `results`. Three more refusals sit in the runner: `_load` rejects a row whose `kind` no metric scores, because such a row still counts toward `n_examples` and the report would look evidenced while the metric selected nothing; `_fraction` scores an empty selection `0.0`, never `1.0`, with `_warn_unmeasured` naming the metric on stderr so the zero explains itself; and `assert_covers` reconciles the rubrics with the scored set. `tests/unit/test_not_falsely_green.py` falsifies all seven metrics against the SHIPPED predicates. The promotion bundle default was `"local"`, which the authority does not register, so an unset `PORTAL_BUNDLE_ID` would have failed closed on `UnknownMetricError`; it is now the registered `journey-portal`. |
| E5 the gate command is this repo's own | PASS | `make check` (`lint typecheck test eval evals-doc-check`) is the recipe and is what CI runs. |
| F4/F5 narrative quality and model replay | N-A | Recorded as n/a rather than left blank: a blank row reads as an omission, n/a with a reason reads as a decision. Nothing here generates text. The portal composes journeys, routes requests, forwards a verified identity and emits audit events, all deterministically, so there is no model to judge and no model output to replay. The eval harness is instead re-purposed to score security and composition invariants, which is the right reading of an eval for a trust boundary. |
...
| F4 local and GCP proved as a PAIR | **PASS** | **17/17 compared fields agree, measured 2026-08-30 against the rebuilt asia-southeast1 deployment on `main` `75eeeb0`, with `pairing.EXEMPT` untouched**: both legs pass all nine steps, both at band `medium` and score `0.25`, both grounded on the case's own uploaded evidence, and `make e2e-pair` exits zero. Closing the 3-field FAIL below took five changes, each proved by execution, none an exemption: the laptop's live profile dropped its local model and seeded demo corpus (Gemini-only, cdd-sow-research#60), the deployment's watchlist fixture was replaced with the real synced snapshot, an orphaned index document with no custody record behind it was purged (it was the 1-vs-2 citation divergence), the model was made to state the exact evidenced amount with the domain computing the band (cdd-sow-research#61/#62, after the model's own banding put `SGD 3,150,000` in `3m-5m` one run and `1m-3m` the next), and Cloud Run's request timeout rose to 900s (journey-portal#73) because the deployed build takes ~4-5 minutes against the real watchlist and the edge was answering 504 while the build completed uselessly behind it. That ~4-5m-vs-~50s runtime gap is the one residue, open in the org backlog: screening measured 1.5s and is not the cause, and the request exports no spans, so it is instrument-first. The FAIL this supersedes is kept below because the route is the evidence. First F4 score in the fleet, and the first honest one. `make e2e-pair` (`e2e/pairing.py`, `e2e/pair_report.py`) compares the deterministic half of the dossier captured from the console's own `POST /v1/cdd` on both targets and exits non-zero on divergence; `tests/test_pairing.py` gates it offline, proving each class of policy divergence is caught and each declared reduction tolerated. **Superseded 2026-08-30 by the PASS above. Measured 2026-08-29 against the rebuilt asia-southeast1 deployment: both legs pass all nine steps, both at band `medium` and score `0.25`, and the pair then FAILED on 3 of 17 compared fields with `pairing.EXEMPT` untouched.** The three are one cause: `sow.value_bands` differs USD against SGD, and `rating.citations` and `sow.citations` differ 1 against 2, because the laptop answers partly from the fictional corpus it seeds for an offline demo while the deployment reads the case's own uploaded evidence. That is a product decision to close — either the seeded corpus stops competing with a case's documents, or it becomes a declared reduction with its reasoning recorded — and it is NOT a comparator fault. What agrees is the half a reviewer acts on: band, score and every weighted factor, by construction. A previous run exited zero on **16 compared fields, twice on independent runs** (2026-08-27), with the exemption list not edited to get there. **That score is withdrawn on 2026-08-29, for two independent reasons, and its withdrawal is why the current FAIL is the stronger result.** The deployment those runs were made against was deleted, and a review found three of the sixteen fields reading keys the dossier wire model does not carry: `sow.amounts` took `amount`, `currency` and `period` from a source object that has only `kind`, `description`, `est_value_band` and `citations`; `rating.factors` read a factor's outcome from `value` and `band`; `screening.alerts` read `list` and `open`. Each compared null to null, so the money in particular was never measured at all. The comparator now reads `est_value_band`, `present` and `entry.source` with `status`, and `load_run` refuses an artifact its run record does not vouch for. A fresh pair has since been run against the rebuilt deployment and is the FAIL recorded at the top of this row; nothing on disk could have settled it, and `load_run` now enforces that. **Scored FAIL first, on purpose, and the route is the evidence.** The first live run (2026-08-25) found **12 divergences over 18 compared fields**: the deployment performed NO sanctions/PEP screening while the laptop screened six lists and neither surface said so; the same subject and the same evidence produced a different verdict (`rating.band` medium vs low, `rating.score` 0.45 vs 0.1) off entirely different scorecard factors; UBO resolution returned an owner locally and none on the deployment; and every managed citation title decayed into its own opaque document id, so a cited source read as `doc-c9dba9861a1f` rather than as the bank statement. Closing them took repository fixes, deployment fixes and two data repairs rather than tolerance: the citation title got one home in the domain, document ids became stable so a corpus stopped growing a copy per run, the demo corpus stopped competing with a case's own evidence, the watchlist snapshot was actually written and made readable by the serving identity, and — the one that mattered most — `rate()` stopped reading the band and score out of the model response, because a band produced by a model is not policy and the published claim is that policy does not change between profiles. A grounded search returning a real prosecution about an unrelated company then had to be gated deterministically by `finding_names_subject`. The last blocker was reproducibility: the shared grounded request builder defaulted to `temperature=0.2`, so the managed profile did not agree with ITSELF run to run, and a green pair would have been luck. Pinned to 0.0, rebuilt, and measured rather than asserted. **Two fields remain exempt with written reasons** (the source-of-wealth classification and its confidence; ownership resolution), both declared quality in the same class as `adverse_media`, and both narrowed rather than wholesale: `sow.value_bands` (which replaced the dead `sow.amounts` above), `sow.present` and `ownership.root_entity` stay compared, so a profile that extracted nothing or resolved no ownership is still caught. **Not yet compared: audit correlation.** Each surface's trail is proved on its own; the two are not yet proved to describe the same run. |

## G. Documentation and adoption

| Check | Verdict | Evidence |
|---|---|---|
| G1 authority order | PASS | `AGENTS.md`/`AGENTS.md` declare Spec, Architecture, Compliance, README; shipped cloud and `cdd-sow-research` mode statements were reconciled. |

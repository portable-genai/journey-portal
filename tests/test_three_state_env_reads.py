...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    "LIVE_IAP_ID_TOKEN": (
        "scripts/live_profile_check.py is intentionally stdlib-only because the hosted GCP "
        "workflow runs it before installing the repo. Its local helper explicitly branches on "
        "None, rejects an empty/whitespace token, and returns only a nonempty credential."
    ),
    "PORT": (

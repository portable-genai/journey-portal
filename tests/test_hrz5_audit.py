...
def _adapter(url: str = "https://hrz5.example.test") -> PlatformAccessAuditAdapter:
...
def test_domain_mapping_is_content_free_deterministic_and_hrz5_compatible() -> None:
    ...
    first = to_hrz5_audit_event(event)
    repeated = to_hrz5_audit_event(event)
...
def test_denied_policy_event_maps_to_blocked_decision() -> None:
    assert to_hrz5_audit_event(_event(action="embed-policy:denied"))["decision"] == "blocked"
    assert (
        to_hrz5_audit_event(_event(action="embed-policy:denied-preflight"))["decision"] == "blocked"
    )
...
def test_platform_adapter_posts_exact_contract_with_workload_token(
    ...
    assert captured["url"] == "https://hrz5.example.test/v1/audit"
    assert captured["headers"] == {
        "authorization": "Bearer token-for:https://hrz5-audience.example.test"
    }
    assert captured["payload"] == to_hrz5_audit_event(_event())
...
@pytest.mark.parametrize(
    ("setting", "value"),
    [
        ("observability_url", "https://hrz5.example.test/path"),
        ("observability_url", "https://hrz5.example.test:443"),
        ("observability_url", "https://HRZ5.example.test"),
        ("observability_audience", "https://hrz5-audience.example.test/path"),
